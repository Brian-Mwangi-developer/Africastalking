...
app.post('/response-callback', async (req, res) => {
  ...
  const message="Welcome to Africa's Talking Call Center How may we assist you today?"
...
app.post('/voice-callback',async (req,res) =>{
    try{
        ...
        if(direction === 'Inbound'){
            callActions = await atVoice.voice_app({
                textPrompt:"Welcome to Africa's Talking Call Center How may we assist you today?",
                finishOnKey:"#",
                timeout:15,
                callbackUrl:`${APP_URL}/talking-center`
            })
...
app.post("/make-call", async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: "Failed to initiate the call." });
  }
});
    ...
    switch (pressedKey) {
      case 1:
        callActions = atVoice.recordAudio({
          introductionText: "Can you describe your emergency and then press the hashkey, after the Beep",
